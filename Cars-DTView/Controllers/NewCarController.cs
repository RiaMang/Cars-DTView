...
namespace Cars_DTView.Controllers
{
    public class NewCarController : Controller
    {
            ...
            string year = "2000", string make = "", string model = "", string trim = "")
        {
            ...
            return Json(new DataTablesResponse(request.Draw, paged, cars.Count(), totalCount),
                JsonRequestBehavior.AllowGet);

        }

        //public PartialViewResult Table(string year)
        //{
        ...
        //    }
        //    List<Car> cars = new List<Car>();
        //    cars = db.Cars.Where(c => c.model_year == year).ToList();
        //    //ViewBag.year = new SelectList( db.Cars.Select(c=>c.model_year).Distinct(), year);
        //    return PartialView("_CarTable",cars);
        //}
        // GET: Cars1/Details/5
